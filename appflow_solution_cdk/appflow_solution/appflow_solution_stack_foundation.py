...
class AppflowSolutionStackFoundation(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        curatedBucketName = CfnParameter(self, "CuratedBucketName", type="String",
                                         ...
                                         allowed_pattern="(?!(^xn--|.+-s3alias$))^[a-z0-9][a-z0-9-]{1,61}[a-z0-9]$")
        resultsBucketName = CfnParameter(self, "ResultsBucketName", type="String",
        ...
        glueDatabaseName = CfnParameter(self, "GlueDatabaseName", type="String",
                                        description="Name of the Glue Database")
        athena_wg_name = CfnParameter(self, "AthenaWGName", type="String",
                                      description="Name of the Athena Workgroup")
        # s3 buckets
        raw_bucket = s3.Bucket(
            self, "RawBucket", bucket_name=rawBucketName.value_as_string)
        curated_bucket = s3.Bucket(
            self, "CuratedBucket", bucket_name=curatedBucketName.value_as_string)
        results_bucket = s3.Bucket(
            self, "ResultsBucket",
            bucket_name=resultsBucketName.value_as_string)
        # Glue Database
        glue_db = glue.CfnDatabase(self, "GlueAppFlowDB", catalog_id=aws_cdk.Aws.ACCOUNT_ID,
                                   database_input=glue.CfnDatabase.DatabaseInputProperty(
                                       name=glueDatabaseName.value_as_string))
        # IAM
        appflow_s3_policy = iam.Policy(self, "appflow_s3_policy", policy_name='appflow_s3_solutionslibrary_policy',
        ...
        CfnOutput(self, "appflow_raw_bucket_name_output", value=raw_bucket.bucket_name,
        ...
        CfnOutput(self, "appflow_athena_wg_name_output", value=athena_wg.name)
